...
var BraveSearchProvider = class {
	...
	async apiKey(options, signal) {
		...
		throw new WebError(`Brave search has no API key for "${options.apiKeyEnv ?? "BRAVE_API_KEY"}"; store it through the credentials service, export it in the launching environment, or set a literal "apiKey" in the web-search-brave config`, "WEB_PROVIDER_CREDENTIAL_MISSING");
	}
	/**
 ...
 * its subscription token from the credentials service under the configured
 * reference (default `BRAVE_API_KEY`), from the launching environment, or from
 * a literal `apiKey` in the `web-search-brave` settings section.
 ...
 * every value it reads is already fully defaulted. The proxy falls back to
 * the launch environment's `HTTPS_PROXY` / `HTTP_PROXY` (Node's fetch does
 * not read proxy variables itself, and a polluted resolver can otherwise make
 * the direct connection fail).
...
export { BRAVE_DEFAULT_BASE_URL, BRAVE_DEFAULT_COUNT, BRAVE_MAX_COUNT, BRAVE_PROVIDER_ID, BraveSearchProvider, Config, WEB_SEARCH_BRAVE_SETTINGS_NAMESPACE, apply, inject, name };
